...
1. **지면 완성도를 잡는다** ← 최우선
   ...
   > **대전제 — 원천은 사이트다.**
   > PDF 는 `index.html` 과 `projects/*.html` 의 **내용과 디자인 언어를 그대로 옮긴 것**이다.
   > 사용자 요구: "지금 사이트의 디자인 및 내용을 유지", "디자인과 내용을 모두 유지해야 돼".
   > 참고용 PPTX 는 **지면이 어떻게 짜였는지만** 보는 자료다.
   > 거기서 디자인·색·서체·문구·구조를 가져오지 않는다. 베끼면 안 된다.
   > 판단이 갈리면 **언제나 사이트를 따른다.**

   사용자가 말한 "구성"은 **장 구조나 서사가 아니다.** 지면이 제대로 짜였는지를 말한다.
   이전 세션에서 이걸 서사 문제로 잘못 알아듣고 장 표지·한 문장 쪽을 넣었다.
   방향을 착각하지 말 것.

   **고쳐야 할 세 가지.**

   1) **잘림** — 내용이 쪽 경계에서 잘려 나간다. 바닥글을 뚫고 나가는 쪽이 있었다.
      원인은 대부분 측정과 인쇄의 불일치다(6절 1·2번). 숫자로 "넘치는 쪽 없음"이
      나와도 **반드시 눈으로 확인한다.** 헤드리스로 쪽을 이미지로 뽑아 본다.

   2) **쓸데없이 큰 여백** — 절반도 안 찬 쪽, 아래가 텅 빈 쪽이 남는다.
      원인은 `newpage`/`softpage` 강제와 마지막 묶음이 짧게 남는 것.
      한 쪽이 어중간하게 비면 앞뒤와 합치거나 담는 양을 조절한다.
      여백은 의도한 자리에만 둔다. 남아서 생긴 여백은 여백이 아니라 미완성이다.

   3) **포트폴리오에 어울리지 않는 디자인** — 문서·보고서처럼 보인다.
      화면(이미지)이 작고 글이 많다. 포트폴리오는 만든 것을 보여주는 문서다.
      화면을 크게, 글은 짧게. 쪽마다 무엇을 보여줄지 먼저 정하고 그만큼만 담는다.

   **참고 자료** `C:\Users\USER\OneDrive\Desktop\3학년 A반 정상연_포트폴리오(V2).pptx`
   (25장 · 이미지 49개 — 장당 평균 두 장). 지면이 어떻게 짜였는지 보고 배운다.
   판형을 따라하라는 뜻이 아니다.
   또 다른 참고 `C:\Users\USER\OneDrive\Desktop\2024001910정상연_이력서.pdf` (A4 세로 10쪽)

   **판형은 별도 결정 사항.** 지금 A4 가로. 16:9로 갈지 A4 세로로 갈지 유지할지 먼저 정한다.

   현재 68쪽. 쪽수를 줄이는 게 목적이 아니라 **모든 쪽이 제대로 짜여 있는 게** 목적이다.
